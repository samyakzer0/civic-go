/**
 * Text Similarity Service for CivicGo Report Grouping
 * 
...
// Common civic infrastructure terms that should have higher weight
const CIVIC_KEYWORDS = new Set([
  'water', 'electricity', 'power', 'road', 'street', 'bridge', 'sidewalk',
  'drainage', 'sewer', 'garbage', 'waste', 'traffic', 'signal', 'light',
  'pothole', 'crack', 'broken', 'damaged', 'leak', 'flood', 'overflow',
  'maintenance', 'repair', 'fix', 'replace', 'clean', 'clear', 'blocked',
  'clogged', 'missing', 'vandalism', 'graffiti', 'safety', 'hazard',
  'emergency', 'urgent', 'park', 'playground', 'fence', 'gate', 'parking',
  'meter', 'sign', 'pole', 'tree', 'branch', 'fallen', 'construction',
  'noise', 'pollution', 'smell', 'odor', 'pipe', 'cable', 'wire',
  'manhole', 'cover', 'pavement', 'asphalt', 'concrete', 'curb'
]);
...
export class TextSimilarityService {
  ...
  private createDocumentVector(text: string): DocumentVector {
    ...
    for (const token of stemmedTokens) {
      ...
      if (CIVIC_KEYWORDS.has(token) && !importantTerms.includes(token)) {
  ...
  public extractCivicTerms(text: string): string[] {
    ...
    for (const token of tokens) {
      if (CIVIC_KEYWORDS.has(token) && !civicTerms.includes(token)) {
