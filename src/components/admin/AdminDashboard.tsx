...
import { ArrowLeft, Layers, Loader2, TrendingUp, ClipboardList, RefreshCw, Shield } from 'lucide-react';
import { getCurrentUser, getUserAdminCategories } from '../../services/supabase.ts';
import { useTheme } from '../../contexts/ThemeContext';
import CategoryAdmin from './CategoryAdmin';
import AdminAnalytics from '../AdminAnalytics';
import ProofExplorerSimplified from '../ProofExplorerSimplified';
...
function AdminDashboard({ onNavigate, user }: AdminDashboardProps) {
  ...
  const [currentView, setCurrentView] = useState<'categories' | 'analytics' | 'tasks' | 'proof'>('analytics'); // Default to analytics
      ...
      <div className="p-6 pb-24 max-w-7xl mx-auto">
        ...
        <div className={`mb-6 p-2 rounded-xl ${theme === 'dark' ? 'bg-gray-800/60' : 'bg-white/60'} backdrop-blur-xl shadow-lg border ${theme === 'dark' ? 'border-gray-700/50' : 'border-gray-200/50'}`}>
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
            ...
            >
              <Shield size={18} />
              <span className="hidden sm:inline">Proof Explorer</span>
